...
function AppRow({ app, index }: { app: TopAppData; index: number }) {
  ...
  return (
    <motion.div
      ...
      className={`p-3 rounded-lg border transition-all hover:bg-secondary/20 ${
        app.isCrypto 
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-3">
          ...
          <div className="w-12 h-12 rounded-lg overflow-hidden">
            ...
            />
            <div className="hidden w-full h-full bg-primary/20 text-primary text-lg font-bold flex items-center justify-center rounded-lg">
              {app.appName.charAt(0)}
            </div>
          </div>
          <div className="flex-1">
            <div className="flex items-center space-x-2 mb-1">
              <span className={`font-medium ${app.isCrypto ? 'text-primary' : 'text-white'}`}>
                {app.appName}
              </span>
              {app.isCrypto && (
              ...
              <span className={`px-2 py-0.5 text-xs rounded font-medium ${getScoreColor(app.score)}`}>
                {app.score}
              </span>
            </div>
        ...
        <div className="text-right">
          <div className="text-xs text-muted-foreground mb-1">Global #{app.globalRank}</div>
      ...
      <div className="mt-3 pt-2 border-t border-border/50">
        <div className="grid grid-cols-3 gap-2 text-xs">
          <div className="text-center">
            ...
            <div className="font-medium text-white">#{app.categoryRank}</div>
            {app.categoryChange24h !== 0 && (
              <div className={`${getChangeColor(app.categoryChange24h)}`}>
                {app.categoryChange24h > 0 ? '+' : ''}{app.categoryChange24h}
              </div>
            )}
          </div>
          <div className="text-center">
            <div className="text-muted-foreground">Global</div>
            <div className="font-medium text-white">#{app.globalRank}</div>
            {app.globalChange24h !== 0 && (
              <div className={`${getChangeColor(app.globalChange24h)}`}>
                {app.globalChange24h > 0 ? '+' : ''}{app.globalChange24h}
              </div>
            )}
          </div>
          <div className="text-center">
            <div className="text-muted-foreground">Score</div>
            <div className={`font-bold ${getScoreColor(app.score).split(' ')[0]}`}>
              {app.score}/100
            </div>
            <div className="text-muted-foreground">
              {app.isCrypto ? '+15 crypto' : 'standard'}
            </div>
          </div>
        </div>
      </div>
